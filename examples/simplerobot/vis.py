...
def run_visualization(robot, window_size=(800, 800), meters_per_pixel=0.02):
    """Run pygame visualization on MAIN thread (required on macOS)."""
    from dimos.msgs.geometry_msgs.Twist import Twist
    import pygame

    pygame.init()
    screen = pygame.display.set_mode(window_size)
    pygame.display.set_caption("Simple Robot — Arrow Keys to Move, Q to Quit")
    ...
    while running:
        ...
        robot._on_twist(Twist(linear=(linear, 0, 0), angular=(0, 0, angular)))
        ...
        # Controls hint
        hints = ["↑↓: forward/back", "←→: rotate", "Q/ESC: quit"]
        for i, text in enumerate(hints):
            screen.blit(small_font.render(text, True, TEXT), (w - 180, 10 + i * 20))
