...
if __name__ == "__main__":
    ...
    try:
        if not args.headless:
            from vis import run_visualization  # runs on MAIN thread (required on macOS)
            robot_thread = threading.Thread(target=_run_robot, daemon=True)
            robot_thread.start()
            run_visualization(robot)  # blocks until window closes
            stop_event.set()
        else:
            _run_robot()
    except KeyboardInterrupt:
