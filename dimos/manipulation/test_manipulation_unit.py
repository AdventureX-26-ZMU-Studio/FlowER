...
class TestStateMachine:
    ...
    def test_begin_planning_rejects_stale_depth_feed(self, robot_config):
        module = _make_module()
        module._world_monitor = MagicMock()
        module._robots = {"test_arm": ("robot_id", robot_config, MagicMock())}
        module._depth_obstacle_source = MagicMock()
        module._depth_obstacle_source.is_fresh.return_value = False
        module._depth_obstacle_source.status.return_value.last_error = "camera timeout"

        assert module._begin_planning() is None
        assert "camera timeout" in module.get_error()
        assert module._state == ManipulationState.IDLE
...
class TestExecute:
    ...
    def test_execute_revalidates_path_after_depth_update(
        self, robot_config, simple_trajectory
    ):
        module = _make_module()
        module._robots = {"test_arm": ("robot_id", robot_config, MagicMock())}
        module._planned_paths = {"test_arm": [JointState(position=[0.0, 0.0, 0.0])]}
        module._planned_trajectories = {"test_arm": simple_trajectory}
        module._world_monitor = MagicMock()
        module._world_monitor.is_path_valid.return_value = False

        assert module.execute() is False
        assert module._state == ManipulationState.FAULT
        assert "depth obstacle" in module.get_error()
        assert module._planned_paths == {}
        assert module._planned_trajectories == {}

    def test_execute_rejects_stale_depth_feed(self, robot_config, simple_trajectory):
        module = _make_module()
        module._robots = {"test_arm": ("robot_id", robot_config, MagicMock())}
        module._planned_trajectories = {"test_arm": simple_trajectory}
        module._depth_obstacle_source = MagicMock()
        module._depth_obstacle_source.is_fresh.return_value = False
        module._depth_obstacle_source.status.return_value.last_error = "stale sensor"

        assert module.execute() is False
        assert module._state == ManipulationState.FAULT
        assert "stale sensor" in module.get_error()
