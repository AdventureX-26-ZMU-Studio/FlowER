...
class ManipulationModuleConfig(ModuleConfig):
    ...
    # Optional fail-closed DCW depth obstacle feed. When configured, planning
    # and execution are refused whenever the feed is stale.
    depth_obstacles: DepthObstacleSourceConfig | None = None
...
class ManipulationModule(Module):
    ...
    def __init__(self, **kwargs: Any) -> None:
        ...
        self._depth_obstacle_source: DepthObstacleSource | None = None
    ...
    def _initialize_planning(self) -> None:
        ...
        depth_config = self.config.depth_obstacles
        if depth_config is not None:
            robot = self._robots.get(depth_config.robot_name)
            if robot is None:
                raise ValueError(
                    f"Depth obstacle robot '{depth_config.robot_name}' is not configured"
                )
            depth_robot_id = robot[0]
            self._depth_obstacle_source = DepthObstacleSource(
                depth_config,
                add_obstacle=self._world_monitor.add_obstacle,
                update_obstacle_pose=self._world_monitor.update_obstacle_pose,
                remove_obstacle=self._world_monitor.remove_obstacle,
                get_mount_pose=lambda: self._world_monitor.get_ee_pose(depth_robot_id),
            )
            self._depth_obstacle_source.start()
            logger.info(
                "DCW depth obstacle source started",
                url=depth_config.url,
                robot=depth_config.robot_name,
            )
    ...
    def _begin_planning(
        ...
        if (robot := self._get_robot(robot_name)) is None:
            ...
            return None
        if self._depth_obstacle_source is not None and not self._depth_obstacle_source.is_fresh():
            status = self._depth_obstacle_source.status()
            detail = status.last_error or "waiting for a current depth frame"
            self._record_error(f"DCW depth obstacle feed is stale: {detail}")
            return None
        with self._lock:
    ...
    def execute(self, robot_name: RobotName | None = None) -> bool:
        ...
        if self._depth_obstacle_source is not None and not self._depth_obstacle_source.is_fresh():
            status = self._depth_obstacle_source.status()
            detail = status.last_error or "depth data timed out"
            return self._fail(f"Refusing execution: DCW depth obstacle feed is stale: {detail}")
        path = self._planned_paths.get(robot_name)
        if (
            path is not None
            and self._world_monitor is not None
            and not self._world_monitor.is_path_valid(robot_id, path)
        ):
            self._planned_paths.pop(robot_name, None)
            self._planned_trajectories.pop(robot_name, None)
            return self._fail(
                "Refusing execution: the planned path now intersects a depth obstacle"
            )
    ...
    def stop(self) -> None:
        ...
        if self._depth_obstacle_source is not None:
            self._depth_obstacle_source.stop()
            self._depth_obstacle_source = None
