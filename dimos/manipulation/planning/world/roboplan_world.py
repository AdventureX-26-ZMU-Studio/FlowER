...
class RoboPlanWorld:
    ...
    def update_obstacle_pose(self, obstacle_id: str, pose: PoseStamped) -> bool:
        ...
        if obstacle_id not in self._obstacles:
            return False
        scene = self._require_scene()
        parent_frame = self._geometry_parent_frame()
        scene.updateGeometryPlacement(obstacle_id, parent_frame, pose_to_matrix(pose))
    ...
    def _geometry_parent_frame(self) -> str:
        if len(self._robots) != 1:
            raise RuntimeError("RoboPlanWorld geometry requires exactly one robot")
        robot = next(iter(self._robots.values()))
        parent_frame = robot.config.base_link
        if not parent_frame:
            raise ValueError("RoboPlanWorld geometry requires a configured robot base_link")
        return parent_frame
    ...
    def _add_obstacle_to_scene(self, obstacle: Obstacle, obstacle_id: str) -> None:
        ...
        if obstacle.obstacle_type == ObstacleType.SPHERE:
            ...
            scene.addSphereGeometry(
                obstacle_id, parent_frame, roboplan_core.Sphere(radius), matrix, color
            )
            return
