...
class InProcessViserAdapter:
    ...
    def get_gripper_position(self, robot_name: RobotName | None = None) -> float | None:
        position = self._module.get_gripper(robot_name)
        return None if position is None else float(position)

    def set_gripper_position(self, position: float, robot_name: RobotName | None = None) -> bool:
        return self._module.set_gripper(float(position), robot_name).is_success()
