...
def test_gui_face_follow_controls_show_status_and_toggle_service(
    make_panel: Callable[..., ViserPanelGui],
    mocker,
) -> None:
    status_read = threading.Event()
    status_rendered = threading.Event()
    toggle_applied = threading.Event()
    status = {
        ...
        "face_center_x": 0.6,
        "face_center_y": 0.4,
        "tracking_error_x": 0.2,
        "tracking_error_y": -0.2,
        "tracking_pitch_speed": -0.05,
        "tracking_yaw_speed": -0.06,
        "tracking_enabled_seconds": 2.0,
    }
    def read_status_request(_gui: ViserPanelGui) -> dict[str, object]:
        status_read.set()
        return status

    def toggle_request(_gui: ViserPanelGui, enabled: bool) -> dict[str, object]:
        toggle_applied.set()
        return {"enabled": enabled}

    read_status = mocker.patch.object(
        ViserPanelGui, "_read_face_follow_status", autospec=True, side_effect=read_status_request
    )
    set_enabled = mocker.patch.object(
        ViserPanelGui,
        "_request_face_follow_enabled",
        autospec=True,
        side_effect=toggle_request,
    )
    original_set_handle_value = ViserPanelGui._set_handle_value
    ...
    def set_handle_value(
        ...
        original_set_handle_value(gui, name, value)
        if name == "face_follow_status" and "Faces: `1`" in value:
            status_rendered.set()
    ...
    mocker.patch.object(
        ...
        "_set_handle_value",
        autospec=True,
        side_effect=set_handle_value,
    )
    server = FakeGuiServer()
    gui = make_panel(
        server,
        make_adapter_with_robot(),
        ViserVisualizationConfig(face_follow_url="http://127.0.0.1:8890"),
    )

    toggle = server.checkboxes["Face follow demo (60 s)"]
    assert status_read.wait(timeout=1.0)
    assert status_rendered.wait(timeout=1.0)

    assert toggle.update_callback is not None
    reads_before_toggle = read_status.call_count
    toggle.update_callback(SimpleNamespace(target=SimpleNamespace(value=True)))

    assert toggle.value is True
    assert toggle_applied.wait(timeout=1.0)
    set_enabled.assert_called_with(gui, True)
    assert read_status.call_count == reads_before_toggle
    assert gui._face_follow_pending is not None
    assert gui._face_follow_pending[0] is True
